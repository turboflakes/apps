...
export default function CoreUsageHeader() {
  ...
  const coreIds = useSelector(state => selectCoreIdsBySession(state, currentSession));
    ...
    <Box
      ...
      <Box sx={{ width: '100%', display: 'flex', justifyContent: 'flex-end'}}>
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', flexWrap: 'wrap', 
          ...
          {[...Array(TOTAL_BARS).keys()].map(i => <CoreBar key={i} highlight={i > TOTAL_BARS - usage_bars - 1} />) }
        </Box>
      </Box>
