...
export default function NetOversubscribedValidatorsBox({sessionIndex, maxSessions}) {
  ...
  const timelineData = data.map((s, i) => ({
    ...
    c100: !isUndefined(s.netstats) ? s.netstats.subsets.filter(f => f.subset === "C100")[0].vals_oversubscribed : 0,
    tvp: !isUndefined(s.netstats) ? s.netstats.subsets.filter(f => f.subset === "TVP")[0].vals_oversubscribed : 0,
    others: !isUndefined(s.netstats) ? s.netstats.subsets.filter(f => f.subset === "NONTVP")[0].vals_oversubscribed : 0,
  }))
