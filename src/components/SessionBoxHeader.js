...
import {
  selectBestBlockAH,
  selectFinalizedBlockAH,
} from "../features/api/blocksAHSlice";
import {
...
export default function SessionBoxHeader({ dark }) {
  ...
  const {
    ...
    isFetching: isFetchingFinalizedBlock,
  } = useGetBlockQuery(
    { blockId: "finalized", show_stats: true },
    { refetchOnMountOrArgChange: true },
  );
  const { isSuccess: isBestBlockSuccess, isFetching: isFetchingBlockSuccess } =
    useGetBlockQuery({ blockId: "best" }, { refetchOnMountOrArgChange: true });
  ...
  if (
    isFetchingFinalizedBlock ||
    isFetchingBlockSuccess ||
    isFetchingSession ||
    isUndefined(session) ||
    isUndefined(best) ||
  ...
  if (!isFinalizedBlockSuccess || !isBestBlockSuccess || !isSessionSuccess) {
    return null;
  }

  // const diff =
  // (isLiveMode
  //   ? finalized.block_number - session.sbix
  //   : session.ebix - session.sbix) + 1;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {isSessionSuccess ? `${session.six.format()}` : "-"}
        </Typography>
      </Box>
      <Box
        sx={{
      ...
      >
        ...
        >
          {isBestBlockSuccess ? `#${best.block_number.format()}` : "-"}
        </Typography>
        <Typography variant="subtitle3">
          {isBestBlockSuccess ? `#${bestAH.block_number.format()}` : "-"}
        </Typography>
      </Box>
      {/* </Box> */}
      {/* {isLiveMode ?
