...
import { pageChanged } from "../features/layout/layoutSlice";
...
function ItemButtom({ address, sessionIndex, rank, diff }) {
  ...
  const grade = useSelector((state) =>
    selectValidatorGradeBySessionAndAddress(state, sessionIndex, address),
  );
  const chainInfo = useSelector(selectChainInfo);

  if (isUndefined(valProfile) || isUndefined(valProfile._performance_ranking)) {
    return (
      <Skeleton variant="text" sx={{ minWidth: 128, fontSize: "0.825rem" }} />
    );
  }
  ...
  return (
      ...
      title={
        <Box sx={{ p: 1 }}>
          <Typography variant="caption" paragraph>
            <b>
              {nameDisplay(
                !!valProfile ? valProfile._identity : stashDisplay(address, 4),
                18,
              )}
            </b>
          </Typography>
          ...
          <Typography component="div" variant="caption" gutterBottom>
            <b>mvr:</b>{" "}
            {Math.round(valProfile._performance_ranking.mvr * 1000000) /
              1000000}
          </Typography>
          <Typography component="div" variant="caption" gutterBottom>
            <b>p/v points (avg):</b>{" "}
            {valProfile._performance_ranking.avg_para_points}
          </Typography>
          <Typography component="div" variant="caption" gutterBottom>
    ...
    >
      ...
      >
        <ListItemIcon
          sx={{ minWidth: 0, mr: 1, display: "flex", alignItems: "center" }}
        >
          <span
            style={{
              width: "4px",
        ...
        <ListItemText
          ...
          sx={{
            color:
              Math.sign(diff) > 0
                ? theme.palette.semantics.green
                : theme.palette.semantics.red,
          }}
...
export default function ValidatorsRankingBox({
  ...
  const { data, isSuccess, isFetching } = useGetValidatorsQuery(
    subset === "TVP" ? { ...params, subset } : params,
    { skip },
  );
  const paramsPrevious = {
    from: sessionIndex - 1 - maxSessions,
    to: sessionIndex - 2,
    ranking: "performance",
    size: RANK_SIZE,
  };
  const {
    data: previousData,
    isSuccess: isPreviousSuccess,
    isFetching: isPreviousFetching,
  } = useGetValidatorsQuery(
    subset === "TVP" ? { ...paramsPrevious, subset } : paramsPrevious,
    { skip },
  );
    ...
    >
      <Box sx={{ display: "flex", justifyContent: "space-between" }}>
        <Box sx={{ display: "flex", flexDirection: "column" }}>
          <Typography variant="h6">Top 100 Validators Performances</Typography>
          ...
          <Tooltip
            ...
            title={
              <Box sx={{ p: 1 }}>
                ...
                <Typography component="div" variant="caption" gutterBottom>
                  <b>score:</b> Backing Votes Ratio (1-MVR) make up 50% of the
                  score, Bitfields Availability Ratio make up 25%, average p/v
                  points make up 18% and number of sessions as p/v the remaining
                  7%
                </Typography>
                <Typography component="div" variant="caption" gutterBottom>
                  <b>sorting:</b> Validators are sorted by Score in descending
                  order
                </Typography>
                <Typography component="div" variant="caption" paragraph>
                  <b>inclusion:</b> To be considered for the ranking, validators
                  must have been p/v for at least 5 times in the last{" "}
                  {maxSessions} sessions.
                </Typography>
                <i>
                  performance_score = (1 - mvr) * 0.50 + bar * 0.25 + ((avg_pts
                  - min_avg_pts) / (max_avg_pts - min_avg_pts)) * 0.18 +
