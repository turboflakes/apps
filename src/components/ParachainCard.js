...
export default function ParachainCard({ sessionIndex, paraId }) {
  ...
  const parachain = useSelector((state) =>
    selectParachainBySessionAndParaId(state, sessionIndex, paraId),
  );
  const mvr = useSelector((state) =>
    selectParachainMvrBySessionAndParaId(state, sessionIndex, paraId),
  );
  const backingPoints = useSelector((state) =>
    selectParachainBackingPointsBySessionAndParaId(state, sessionIndex, paraId),
  );
  ...
  const validityVotes =
    parachain.stats.ev + parachain.stats.iv + parachain.stats.mv;
  const pieChartsData = createBackingPieData(
    parachain.stats.ev,
    parachain.stats.iv,
    parachain.stats.mv,
    paraId,
  );
  const chainName = paraId
    ? isChainSupported(selectedChain, paraId)
      ? getChainName(selectedChain, paraId)
      : paraId
    : "";
      ...
      <Box sx={{ height: "72px", p: 2 }}>
        <Box sx={{ mb: 1, display: "flex", justifyContent: "space-between" }}>
          {!!chainName ? (
            ...
            >
              <img
                src={getChainLogo(selectedChain, paraId)}
                style={{
                  width: 32,
                  height: 32,
                  marginRight: 8,
      ...
      <Box sx={{ py: 2, display: "flex", justifyContent: "space-around" }}>
        ...
        >
          <Typography variant="caption" sx={{ ml: 3 }}>
            {!isNull(parachain.group)
              ? `Backed by Val. Group ${parachain.group}`
              : `Waiting core assignment`}
          </Typography>
