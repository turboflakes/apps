...
import { SUBSET } from "../features/api/validatorsSlice";
import { selectPoolById } from "../features/api/poolsMetadataSlice";
import { selectNomineesBySessionAndPoolId } from "../features/api/poolsSlice";
import { stakeDisplay, symbolDisplay } from "../util/display";
...
export default function PoolCard({ sessionIndex, poolId }) {
  ...
  const nominees = useSelector((state) =>
  ...
  const groupedBySubset = groupBy(nominees, (v) =>
    !isUndefined(v.profile) ? v.profile.subset : "NONVAL",
  );
  const subsetPieData = orderBy(
  ...
  return (
    ...
    >
      ...
      >
        <Box sx={{ mb: 1 }}>
          <Tooltip title={pool.metadata}>
            ...
            >
              <span style={{ marginRight: "8px" }}>#{poolId}</span>
              {pool.metadata}
            </Typography>
          </Tooltip>
      ...
      />
      {showInsights ? (
        <PoolHistorySection sessionIndex={sessionIndex} poolId={poolId} />
      ) : (
        <Box>
          <Box sx={{ p: 2, display: "flex", justifyContent: "space-between" }}>
            ...
            <Box sx={{ display: "flex", flexDirection: "column" }}>
              <Typography variant="caption" align="left" sx={{ ml: 3 }}>
                active: {nActive}
              </Typography>
              <PoolActiveNomineesList
          ...
          >
            ...
            >
              ...
              <Typography variant="h5" align="center">
                {stakeDisplay(rewards, selectedChainInfo, 4, true, true, true, {
                  ...theme.typography.h6,
                })}
              </Typography>
            </Box>
            ...
            >
              ...
              <Typography variant="h5" align="center">
                {stakeDisplay(points, selectedChainInfo, 2, true, false)}
              </Typography>
            </Box>
