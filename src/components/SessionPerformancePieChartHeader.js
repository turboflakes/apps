...
export default function SessionPerformancePieChartHeader() {
  ...
  // PERFORMANCE RATIO = (1 - MVR) * 0.75 + BAR * 0.25
  const ratio = (1 - finalized._mvr) * 0.75 + finalized._bar * 0.25;
  const previousRatio =
    (1 - previousFinalized._mvr) * 0.75 + previousFinalized._bar * 0.25;
  const pieData = [
    createData("done", Math.round(ratio * 100)),
    createData("progress", Math.round((1 - ratio) * 100)),
  ];

  const trend = ratio - previousRatio;
        ...
        >
          ...
          <Box sx={{ display: "flex" }}>
            <Typography variant="h6">
              {!isUndefined(finalized._mvr)
                ? `${Math.round((1 - finalized._mvr) * 100)}%`
                : "-"}
              <span
                style={{
              ...
              >
                {!isNaN(trendPer)
                  ? Math.sign(trendPer) !== 0
                    ? `${trendSign(trendPer)} ${Math.abs(trendPer)}%`
                    : ``
                  : ""}
              </span>
            </Typography>
