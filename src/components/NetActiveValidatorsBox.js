      ...
      >
        ...
        <Box sx={{ minWidth: "192px" }}>
          <Typography component="div" variant="caption">
            ...
            ALL: <b>{data.total}</b> (
            {Math.round((data.total * 100) / data.total)}%)
          </Typography>
          {/* <Typography component="div" variant="caption">
            <span
              style={{
            ...
            DN: <b>{data.tvp}</b> ({Math.round((data.tvp * 100) / data.total)}%)
          </Typography>
          <Typography component="div" variant="caption">
            <span
...
export default function NetActiveValidatorsBox({ sessionIndex, maxSessions }) {
  ...
  const timelineData = data.map((s, i) => ({
    ...
    total: !isUndefined(s.netstats)
      ? s.netstats.subsets.map((m) => m.vals_active).reduce((a, b) => a + b, 0)
      : 0,
    // c100: !isUndefined(s.netstats)
    //   ? s.netstats.subsets.filter((f) => f.subset === "C100").length > 0
    //     ? s.netstats.subsets.filter((f) => f.subset === "C100")[0].vals_active
    //     : 0
    //   : 0,
    // tvp: !isUndefined(s.netstats)
    //   ? s.netstats.subsets.filter((f) => f.subset === "TVP").length > 0
    //     ? s.netstats.subsets.filter((f) => f.subset === "TVP")[0].vals_active
    //     : 0
    //   : 0,
    // others: !isUndefined(s.netstats)
    //   ? s.netstats.subsets.filter((f) => f.subset === "NONTVP").length > 0
    //     ? s.netstats.subsets.filter((f) => f.subset === "NONTVP")[0].vals_active
    //     : 0
    //   : 0,
  }));
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between" }}>
        ...
        >
          <Typography variant="caption" gutterBottom>
            Total <b>Active Validators</b> by the end of the previous epoch
          </Typography>
