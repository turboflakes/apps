...
import { addressChanged, selectChainInfo } from "../features/chain/chainSlice";
import { pageChanged } from "../features/layout/layoutSlice";
import { selectNomineesBySessionAndPoolId } from "../features/api/poolsSlice";
...
// });
const gradeValue = (v) =>
  !isUndefined(v.para_summary) && !isUndefined(v.para)
    ? gradeByRatios(
...
function ItemButtom({ validator }) {
  ...
  // const subset = isUndefined(validator.profile)
  //   ? "NONVAL"
  //   : validator.profile.subset;

  return (
    <ListItemButton
      sx={{ borderRadius: 30 }}
      disableRipple
      onClick={() => handleAddressSelected(validator.address)}
      title={nameDisplay(
        !!validator.profile
          ? validator.profile._identity
          : stashDisplay(validator.address, 4),
        64,
      )}
    >
      <ListItemIcon
      ...
      >
        {/*
          // TODO: Add the color of the current grade or
          // previous X grades
          <span
          style={{
            width: "4px",
      ...
      <ListItemText
        sx={{
          whiteSpace: "nowrap",
          textDecoration: isUndefined(validator.profile)
            ? "line-through"
            : "none",
        }}
        primary={nameDisplay(
...
export default function PoolNomineesList({ sessionIndex, poolId }) {
  ...
  const validators = useSelector((state) =>
    selectNomineesBySessionAndPoolId(state, sessionIndex, poolId),
  );
