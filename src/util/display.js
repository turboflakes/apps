...
export const stashDisplay = (stash, max = 6) => {
  return !!stash
    ? `${stash.slice(0, max)}...${stash.slice(stash.length - max, stash.length)}`
    : `-`;
};
...
export const stakeDisplay = (
  ...
  if (!!networkDetails.tokenDecimals[0] && !!networkDetails.tokenSymbol[0]) {
    const networkDecimals = Math.pow(
      10,
      parseInt(networkDetails.tokenDecimals[0], 10),
    );
    if (format && symbol && iu && style) {
      ...
      return (
        <span>
          ...
          <span
            style={{ ...style }}
          >{`${t[1]}${networkDetails.tokenSymbol[0]}`}</span>
        </span>
      );
    }
...
export const versionToHex = (version) => {
  return hslToHex(
    210,
    40,
    convertToPercentage(versionToNumber(version), interval),
  );
};
