...
import {
  ...
  selectMessagesInQueue,
} from "./socketSlice";
import { socketBlockReceived, socketBlocksReceived } from "./blocksSlice";
import { selectSessionCurrent, socketSessionReceived } from "./sessionsSlice";
import {
  socketValidatorReceived,
  socketValidatorsReceived,
  selectValidatorBySessionAndAddress,
} from "./validatorsSlice";
import { socketParachainsReceived } from "./parachainsSlice";
import { selectValidatorBySessionAndAuthId } from "./authoritiesSlice";
import { selectAddress, selectChain } from "../chain/chainSlice";
import { selectPage } from "../layout/layoutSlice";
// import apiSlice from './apiSlice';
import { getNetworkHost } from "../../constants";

const dispatchValidator = (store, address, method = "subscribe") => {
  const msg = JSON.stringify({
    method: `${method}_validator`,
    params: [address],
  });
  ...
  if (!!validator && validator.is_para) {
    const addresses = validator.para.peers.map((peer) => {
      const validatorPeer = selectValidatorBySessionAndAuthId(
        store.getState(),
        currentSession,
        peer,
      );
    ...
    return (next) => {
      return (action) => {
        ...
        if (action.type === "chain/addressChanged") {
          ...
          const previousValidator = selectValidatorBySessionAndAddress(
            store.getState(),
            currentSession,
            previousAddress,
          );
          const nextAddress = action.payload;
          const nextValidator = selectValidatorBySessionAndAddress(
            store.getState(),
            currentSession,
            nextAddress,
          );
          ...
          if (isConnected) {
            if (
              previousAddress !== nextAddress.payload &&
              !isUndefined(previousValidator) &&
              !isUndefined(nextValidator) &&
              previousValidator.is_para &&
              nextValidator.is_para &&
        ...
        if (action.type === "layout/pageChanged") {
          ...
          switch (previousPage) {
            case "parachains/overview": {
              const msg = JSON.stringify({
                method: "unsubscribe_parachains",
                params: [currentSession.toString()],
              });
              store.dispatch(socketActions.messageQueued(msg));
              break;
            }
