    ...
    .addMatcher(matchValidatorsReceived, (state, action) => {
      ...
      forEach(groupedBySession, (validators, session) => {
        ...
        const groups = Object.values(groupedByValGroupId).map(group => ({
          ...
          _core_assignments: group[0].para_summary.ca,
          ...
          _validity_votes: group.map(v => v.para_summary.ev + v.para_summary.iv + v.para_summary.mv).reduce((a, b) => a + b, 0) / group.length,
